...
export function DialogDeleteEntries({ open, prefix, loadKeys }: {
  open: Signal<boolean>;
  ...
  function loadTree() {
    loading.value = true;
    const target = `/api/kv/${keyJsonToPath(prefix?.value ?? [])}?tree`;
    ...
    >
      ...
      <form
        ...
        onSubmit={(evt) => {
          alert.value = undefined;
          deleting.value = true;
          const target = `/api/kv${prefix?.value?.length ? `/${keyJsonToPath(prefix.value)}` : ""}`;
