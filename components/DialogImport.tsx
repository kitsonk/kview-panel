...
export function DialogImport({ open, prefix }: { open: Signal<boolean>; prefix: Signal<KvKeyJSON> }) {
  const target = useComputed(() => prefix.value.length ? `/api/bulk/${keyJsonToPath(prefix.value)}` : `/api/bulk`);
